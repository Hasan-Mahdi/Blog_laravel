...
@section('content')
    <div class="container">
        <h1 class="text-center"><span class="text-warning">Edit {{$post->title}}</span></h1>
        <br>

        <form action="{{ route('posts.update', $post) }}" method="POST" enctype="multipart/form-data" style="max-width:400px; margin:auto">
          @csrf
          @method('PUT') <!-- Uncomment if updating an existing record instead of creating a new one -->
          <div class="mb-3">
              <label for="title" class="form-label">Title</label>
              <input type="text" autofocus class="form-control" value="{{$post->title}}" name="title" id="title">
              @error('title')
